...
setwd("C:/Users/prabindawadi/Desktop/Bioinformatics_Club")
...
png(file.path(plot_dir, "PCA_plot.png"), width = 1000, height = 800)
...
png(file.path(plot_dir, "Boxplot_Normalized_Expression.png"), width = 1200, height = 800)
boxplot(assay(vsd), las = 2, main = "Normalized Expression Values", col = "skyblue")
dev.off()

# Save PCA data as CSV
write.csv(pcaData, file = "PCA_data.csv", row.names = TRUE)

# PERMANOVA to check PCA group separation is statistically significant
dist_matrix <- dist(t(assay(vsd)))  # distance on samples (transpose for dist())
adonis_result <- adonis2(dist_matrix ~ biopsy_site, data = meta, permutations = 4999)

# Print to console (optional)
print(adonis_result)

# Save PERMANOVA results to text file
sink(file.path("PERMANOVA_results.txt"))
cat("PERMANOVA (adonis2) test for biopsy_site group separation\n")
cat("-----------------------------------------------------------\n\n")
print(adonis_result)
sink()
...
# MA Plot - Primary
png(file.path(plot_dir, "MA_plot_Primary_vs_Normal.png"), width = 1000, height = 800)
...
png(file.path(plot_dir, "Volcano_Primary_vs_Normal.png"), width = 1000, height = 800)
ggplot(df_primary, aes(x = log2FoldChange, y = -log10(padj), color = threshold)) +
  geom_point(alpha = 0.7, size = 2) +
  scale_color_manual(values = c("Upregulated" = "green", "Downregulated" = "red", "NS" = "gray")) +
  ...
  labs(title = "Volcano Plot: Primary Tumor vs Normal",
       x = "Log2 Fold Change", y = "-Log10 Adjusted P-value", color = "DEG Status") +
  theme_minimal(base_size = 16)
dev.off()
...
# MA Plot - Liver
png(file.path(plot_dir, "MA_plot_Liver_vs_Normal.png"), width = 1000, height = 800)
plotMA(res_liver, ylim = c(-5, 5), main = "MA Plot: Liver Mets vs Normal")
dev.off()
...
png(file.path(plot_dir, "Volcano_Liver_vs_Normal.png"), width = 1000, height = 800)
